...
namespace TMom.Infrastructure.Helper
{
    public class JsonHelper
    {
        ...
        public static void SetChildNodes(ref JToken json, string nodeName, string value)
        {
            try
            {
                JToken result = json;
        ...
        /// <returns></returns>
        public static string SeleteNode(string jsonStr, string key)
        {
            try
            {
                ...
                string result = "";
                // 正则匹配
                var node = json.SelectToken("$.." + key);
                if (node != null)
                {
                    if (node.Type == JTokenType.Object)
                    {
                        result = node.ObjToString();
                    }
                    else if (node.Type == JTokenType.Array)
                    {
                        result = JsonConvert.SerializeObject(node.Values<object>());
                    }
                    else if (node.Type == JTokenType.String || node.Type == JTokenType.Integer || node.Type == JTokenType.Float)
                    {
                        //返回string值
