...
export const createStorage = ({
  ...
  class Storage {
    ...
    /** 获取key的过期时间戳 */
    private getKeyTimeAndExpire(key: string) {
      const val = uni.getStorageSync(this.getKey(key))
      if (!val) return { time: 0, expire: 0 }

      try {
        const decVal = this.hasEncrypt ? this.encryption.decryptByAES(val) : val
        const data = JSON.parse(decVal)
        const { time, expire } = data
        return { time, expire }
      } catch (e) {
        return { time: 0, expire: 0 }
      }
    }
    ...
    set(key: string, value: any, expire: number | null = timeout) {
      try {
        ...
        if (this.get(key)) {
          const { time, expire } = this.getKeyTimeAndExpire(key)
