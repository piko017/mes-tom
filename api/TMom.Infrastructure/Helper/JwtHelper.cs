    ...
    public class JwtHelper
    {
        ...
        public static TokenModelJwt SerializeJwt(string jwtStr)
        {
            ...
            if (jwtStr.IsNotEmptyOrNull() && jwtHandler.CanReadToken(jwtStr))
            {
                ...
                jwtToken.Payload.TryGetValue(ClaimTypes.GroupSid, out factoryId);
                jwtToken.Payload.TryGetValue(ClaimTypes.PostalCode, out factoryCode);

                tokenModelJwt = new TokenModelJwt
                {
                    Uid = (jwtToken.Id).ObjToInt(),
                    Role = role != null ? role.ObjToString() : "",
                    FactoryId = factoryId != null ? factoryId.ObjToString() : "",
                    FactoryCode = factoryCode != null ? factoryCode.ObjToString() : ""
                };
            }
        ...
        public static bool ValidateToken(string? token)
        {
            ...
            var tokenModel = SerializeJwt(token);
            return tokenModel != null && tokenModel.Uid > 0 && customSafeVerify(token);
        }
    }
