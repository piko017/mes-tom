...
namespace TMom.Infrastructure.Helper
{
    public static class ImageHelper
    {
        ...
        /// <param name="savePath">保存的相对路径, eg: /Images/QRCode/123456.png</param>
        ...
        public static void QRCoder(string text, string savePath, string logoPath = "", int keepWhiteBorderPixelVal = -1)
        {
            ...
            string fullPath = Path.Combine(MomFilePath.BaseFilesPath, savePath);
            var saveDirPath = System.IO.Path.GetDirectoryName(fullPath);
            if (!System.IO.Directory.Exists(saveDirPath))
            {
                System.IO.Directory.CreateDirectory(saveDirPath);
            }
            var fs = new System.IO.FileStream(fullPath, System.IO.FileMode.OpenOrCreate, System.IO.FileAccess.Write, System.IO.FileShare.None);
            fs.Write(bytes, 0, bytes.Length);
            fs.Close();
        }
        ...
        /// 生成验证码
        /// </summary>
        /// <param name="code">生成的验证码code</param>
        /// <param name="width">宽</param>
        /// <param name="height">高</param>
        /// <param name="len">字符数</param>
        /// <param name="bgColor">背景颜色, 默认亮色</param>
        /// <returns></returns>
        /// <exception cref="Exception"></exception>
        public static string GetVerifyCode(out string code, int width = 100, int height = 50, int len = 4, string bgColor = "light")
        {
            try
            {
                List<char> chars = new List<char>();
                string str = "3456789abcdefghjkmnpqrstuvwxyABCDEFGHJKLMNPQRSTUVWXY";
                Random r = new Random();
                for (int i = 0; i < len; i++)
                {
                    char c = str[r.Next(0, str.Length - 1)];
                    chars.Add(c);
                }
                code = new string(chars.ToArray());
                SKBitmap bmp = new SKBitmap(width, height);
                using (SKCanvas canvas = new SKCanvas(bmp))
                {
                    //背景色
                    canvas.DrawColor(bgColor != "realDark" ? SKColors.White : new SKColor(27, 39, 53, 10));

                    using (SKPaint sKPaint = new SKPaint())
                    {
                        sKPaint.TextSize = 25;//字体大小
                        sKPaint.IsAntialias = true;//开启抗锯齿
                        sKPaint.Typeface = SKTypeface.FromFamilyName("微软雅黑", SKFontStyle.Bold);// 微软雅黑 字体在Linux系统下没有  dockerfile中添加
                        SKRect size = new SKRect();
                        sKPaint.MeasureText(chars[0].ToString(), ref size);//计算文字宽度以及高度

                        float temp = (bmp.Width / len - size.Size.Width) / 2;
                        float temp1 = bmp.Height - (bmp.Height - size.Size.Height) / 2;
                        Random random = new Random();

                        for (int i = 0; i < len; i++)
                        {
                            sKPaint.Color = new SKColor((byte)random.Next(50, 200), (byte)random.Next(50, 200), (byte)random.Next(50, 200));
                            canvas.DrawText(chars[i].ToString(), temp + (width / len) * i, temp1, sKPaint);//画文字
                        }
                        //干扰线
                        for (int i = 0; i < 5; i++)
                        {
                            sKPaint.Color = new SKColor((byte)random.Next(0, 255), (byte)random.Next(0, 255), (byte)random.Next(0, 255));
                            canvas.DrawLine(random.Next(0, 50), random.Next(1, 49), random.Next(51, 100), random.Next(1, 49), sKPaint);
                        }
                    }
                    //页面展示图片
                    using (SKImage img = SKImage.FromBitmap(bmp))
                    {
                        ...
                        {
                            string base64Str = Convert.ToBase64String(p.ToArray());
